...
import { supabase, isRealSupabaseConfigured, realSupabase } from '../../lib/supabase';

export default function SettingsPage() {
  const { user, isAdmin, loading: authLoading, setMockRole } = useAuth();
  ...
  const [lowStockThreshold, setLowStockThreshold] = useState<number>(() => {
    return Number(localStorage.getItem('stockflow_setting_low_stock_threshold')) || 10;
  });
  const [maxWarehouseCapacity, setMaxWarehouseCapacity] = useState<number>(() => {
    return Number(localStorage.getItem('stockflow_setting_max_capacity')) || 1000;
  });
  const [soundAlertsEnabled, setSoundAlertsEnabled] = useState<boolean>(() => {
    return localStorage.getItem('stockflow_setting_sound_alerts') !== 'false';
  });
  const [autoRefreshInterval, setAutoRefreshInterval] = useState<number>(() => {
    return Number(localStorage.getItem('stockflow_setting_auto_refresh')) || 5;
  });
  ...
  const playSynthSound = (frequency = 440, duration = 0.15, type: 'sine' | 'square' | 'triangle' | 'sawtooth' = 'sine') => {
    ...
    try {
      ...
      gainNode.gain.setValueAtTime(0.08, audioCtx.currentTime); // low comfortable volume
  ...
  const handleTestDatabase = async (silent = false) => {
    if (!silent) {
      setDbStatus(prev => ({ ...prev, checking: true, message: language === 'id' ? 'Menghubungkan ke database...' : 'Linking to database...' }));
    }
    const startTime = performance.now();
    
    try {
      if (isRealSupabaseConfigured) {
        // Probe live Supabase products table
        ...
        if (error) {
          ...
          // Count products
          const { count: productCount } = await realSupabase
            .from('products')
            .select('*', { count: 'exact', head: true });

          const { count: txCount } = await realSupabase
            .from('transactions')
            .select('*', { count: 'exact', head: true });

          const { count: laundryCount } = await realSupabase
        ...
        // Probe simulated client-side databases
        ...
        const prodC = getLocalCount('stockflow_local_products');
        const txC = getLocalCount('stockflow_local_transactions');
        const laundryC = getLocalCount('stockflow_local_laundry');
        const restockC = getLocalCount('stockflow_local_restock');
        const userC = getLocalCount('stockflow_local_users');
  ...
  const handleSaveSettings = () => {
    ...
    // Dispatch custom settings update event
  ...
  const handleResetLocalDatabase = () => {
    const confirmMessage = language === 'id' 
      ? 'Apakah Anda yakin ingin mengatur ulang database lokal Anda? Semua produk, transaksi, dan laundry kustom akan dihapus dan dipulihkan kembali ke data bibit default.'
      : 'Are you sure you want to reset your local database? All custom products, transactions, and laundry batches will be cleared and restored back to default seed packets.';
...
CREATE TABLE IF NOT EXISTS laundry_records (
  ...
  const runSystemAudit = async () => {
    ...
    try {
      if (isRealSupabaseConfigured) {
        const { data, error } = await realSupabase.from('products').select('id').limit(1);
    ...
    // 4. Data counts
    appendLog('🔍 Indexing current tables rows and inventory quantities...');
    try {
      const getLocalCount = (key: string) => {
        const str = localStorage.getItem(key);
        return str ? JSON.parse(str).length : 0;
      ...
      const pC = getLocalCount('stockflow_local_products');
      const tC = getLocalCount('stockflow_local_transactions');
      const lC = getLocalCount('stockflow_local_laundry');
      const rC = getLocalCount('stockflow_local_restock');
      const uC = getLocalCount('stockflow_local_users');
      ...
      appendLog(`   -> Procure Requests (Restocks)  : ${rC} files`);
      appendLog(`   -> Authorized IDs (Users)       : ${uC} members`);
    } catch (err: any) {
  ...
  const handleLinkWebhook = async () => {
    ...
    try {
      ...
      setSetupResult({
        success: false,
        message: language === 'id' ? 'Gagal memproses pendaftaran webhook.' : 'Failed to register webhook.'
      });
  ...
  const handleTestNotification = async () => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        setSetupResult({
          success: true,
          message: language === 'id' ? 'Notifikasi uji coba berhasil terkirim!' : 'Test alarm notification sent successfully!'
        });
        playSynthSound(600, 0.1, 'sine');
        ...
        setSetupResult({
          success: false,
          message: data.error || (language === 'id' ? 'Gagal mengirimkan notifikasi.' : 'Failed to send notification.')
        });
      ...
      setSetupResult({
        success: false,
        message: 'Error sending test notification pulse.'
      });
      playSynthSound(220, 0.35, 'sawtooth');
    } finally {
      setIsTesting(false);
  ...
  if (!isAdmin) {
    return (
      <div id="unauthorized-settings" className="flex-1 flex flex-col items-center justify-center p-8 max-w-md mx-auto text-center py-20 animate-fade-in">
                    ...
                    <div className={`p-4 rounded-2xl border text-xs leading-relaxed space-y-2 ${
                      ...
                      {dbStatus.success && dbStatus.tableCounts && (
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 pt-2 border-t border-white/5 text-[10px] font-mono text-slate-400">
                          ...
                          {dbStatus.source === 'local-storage' && (
                            <>
                              <div>📝 {language === 'id' ? 'Restock' : 'Restock'}: {dbStatus.tableCounts.restock}</div>
                              <div>👥 {language === 'id' ? 'User IDs' : 'User IDs'}: {dbStatus.tableCounts.users}</div>
                            </>
                          )}
                        </div>
                      )}
        ...
        {activeConfigTab === 'access' && (
          <div className="space-y-6 relative z-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="md:col-span-2 space-y-4">
                <div className="flex items-center gap-4 bg-slate-900/40 p-5 rounded-2xl border border-white/5">
                  ...
                  <div className="space-y-0.5">
                    ...
                    <h4 className="font-bold text-white leading-tight">{user?.email}</h4>
                    <p className="text-[10px] font-mono text-slate-500">ID: {user?.id}</p>
                  </div>
                </div>

                <div className="p-4 bg-slate-900/20 border border-white/5 rounded-2xl text-xs space-y-2 text-slate-400 leading-relaxed">
              ...
              {/* Mock Authority Section for QA testing */}
              <div className="bg-indigo-950/20 p-5 rounded-2xl border border-indigo-500/15 space-y-4 flex flex-col justify-between">
                <div className="space-y-1">
                  <h4 className="text-xs font-bold text-indigo-400 uppercase tracking-wider flex items-center gap-1.5">
                    <UserCheck size={14} />
                    {language === 'id' ? 'Uji Coba Hak Akses' : 'Test Authority Levels:'}
                  </h4>
                  <p className="text-[10px] text-slate-500 leading-normal">
                    {language === 'id' 
                      ? 'Alihkan otoritas Anda sementara untuk menguji performa halaman dan batasan tombol.'
                      : 'Toggle your local role vector temporarily to evaluate conditional button states.'}
                  </p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    onClick={() => {
                      setMockRole('admin');
                      playSynthSound(523.25, 0.1, 'sine');
                    }}
                    className={`px-3 py-2 text-[10px] font-bold uppercase tracking-wider rounded-xl transition-all cursor-pointer ${
                      isAdmin 
                  ...
                  <button
                    onClick={() => {
                      setMockRole('operator');
                      playSynthSound(392.00, 0.1, 'sine');
                    }}
                    className={`px-3 py-2 text-[10px] font-bold uppercase tracking-wider rounded-xl transition-all cursor-pointer ${
                      !isAdmin 
                        ? 'bg-indigo-600 text-white shadow-lg' 
            ...
            <div className="flex items-center justify-between pt-4 border-t border-white/5">
              <div className="text-xs">
                {settingsSaveSuccess && (
                  <span className="text-emerald-400 font-bold flex items-center gap-1 bg-emerald-900/10 px-3 py-1.5 rounded-xl border border-emerald-500/25 animate-fade-in">
                    <CheckCircle2 size={13} />
                    {language === 'id' ? '✓ Pengaturan alert disimpan!' : '✓ Alert specifications updated!'}
                  </span>
                )}
              </div>
