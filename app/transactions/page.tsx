    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-bold text-white tracking-tight">System Transactions</h1>
        </div>
      ...
      <div className="relative group">
        ...
        <input 
          type="text" 
          placeholder="Filter ledger by SKU name, notes, or hash..."
          value={searchQuery}
      ...
      <tr className="bg-white/[0.02] border-b border-white/5">
        ...
        <th className="p-4 sm:p-6 text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] text-left">Item</th>
        <th className="p-4 sm:p-6 text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] text-left hidden sm:table-cell">Flow</th>
        <th className="p-4 sm:p-6 text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] text-left">Qty</th>
        <th className="p-4 sm:p-6 text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] text-left">Timestamp</th>
        <th className="p-4 sm:p-6 text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] text-left hidden sm:table-cell">Operator</th>
        <th className="p-4 sm:p-6 text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] text-left hidden md:table-cell">Context</th>
        <th className="p-4 sm:p-6 text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] text-right">Ops</th>
      </tr>
            </thead>
            <tbody className="divide-y divide-white/[0.03]">
              {isLoading ? (
                <tr>
                  <td colSpan={7} className="p-20 text-center text-slate-500 font-bold uppercase tracking-widest text-[10px]">
                    Accessing Secure Audit Vault...
                  </td>
                </tr>
              ) : filteredTransactions.length === 0 ? (
                <tr>
                  <td colSpan={7} className="p-20 text-center text-slate-600 font-bold uppercase tracking-widest text-[10px]">
                    No historical logs matching criteria
                  </td>
                </tr>
              ) : (
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-4 sm:p-6">
                        <Link href={`/products/${t.product_id}`} className="flex items-center gap-2 sm:gap-3 hover:text-indigo-400 transition-colors">
                          <div className="w-8 h-8 bg-slate-900 rounded-lg hidden xs:flex items-center justify-center border border-white/5">
                            <Package size={14} className="text-slate-500" />
                          </div>
                          <span className="text-xs sm:text-sm font-bold text-white tracking-tight truncate max-w-[80px] sm:max-w-none">{t.products?.name}</span>
                        </Link>
                      </td>
                      <td className="p-4 sm:p-6 hidden sm:table-cell">
                        <div className={`flex items-center gap-2 text-[10px] font-black uppercase tracking-widest ${
                          ...
                          <span className="hidden lg:inline">{t.type === 'in' ? 'RECEIVE' : 'DISPATCH'}</span>
                        </div>
                      </td>
                      <td className="p-4 sm:p-6">
                      ...
                      <td className="p-4 sm:p-6">
                        <div className="flex flex-col">
                          <span className="text-[10px] sm:text-xs font-bold text-slate-300 whitespace-nowrap">{new Date(t.timestamp).toLocaleDateString()}</span>
                          <span className="text-[9px] sm:text-[10px] text-slate-500 font-mono hidden sm:inline">{new Date(t.timestamp).toLocaleTimeString()}</span>
                        </div>
                      </td>
                      <td className="p-4 sm:p-6 hidden sm:table-cell">
